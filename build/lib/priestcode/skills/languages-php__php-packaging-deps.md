---
name: PHP packaging & deps
description: Structure, build, and ship a PHP project and its dependencies.
category: languages/php
tags: [php, packaging, build, dependencies]
---
# PHP packaging & deps

## When to use
Setting up, building, or releasing a PHP project.

## Checklist
- Use the standard build tool and dependency manager with a lockfile.
- Pin versions; separate runtime vs dev deps; produce reproducible builds.

## Pitfalls
- Unpinned dependencies drift and break builds.
- Committing build artifacts.
